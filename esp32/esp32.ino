...
void calibrateSensors() {
  ...
  Serial.print("Baseline S1: "); Serial.print(baseline1); Serial.println(" cm");
  Serial.print("Baseline S2: "); Serial.print(baseline2); Serial.println(" cm");
  ...
  if (currentMillis - lastPingTime >= PING_INTERVAL) {
    ...
    if (pingSensor1Next) {
      if (!s1_Triggered && baseline1 > 0) { 
        currentDist1 = getDistance(TRIG1, ECHO1); // Saved to global for web JSON
        processDistance(1, currentDist1, baseline1, bufferCount1, s1_Triggered, S_LED1);
      }
      pingSensor1Next = false;
    } else {
      if (!s2_Triggered && baseline2 > 0) {
        currentDist2 = getDistance(TRIG2, ECHO2);
        processDistance(2, currentDist2, baseline2, bufferCount2, s2_Triggered, S_LED2);
      }
...
void processDistance(int sensorNum, int currentDist, int baseline, int &bufferCount, bool &triggerFlag, int ledPin) {
